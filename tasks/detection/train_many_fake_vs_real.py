...
if __name__ == "__main__":
    ...
    assert args.dset == "toyforgerynetimages", "currently only toyforgerynet is supported"

    target_size = get_model_target_size(args.model_type)
    task = "detection"

    categories = [int(cat) for cat in args.categories]
    if 0 in categories:
        categories.remove(0)

    for cat in categories:
        print(f"Training category {cat} vs 0 (real)")
